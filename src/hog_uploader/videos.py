...
def load_videos(path: Path, video_extension: str = VideoExtension.MP4) -> list[Path]:
    return sorted(
        file for file in path.iterdir() if file.suffix.lower() == video_extension
    )
...
def get_days_from_concatenated(
    concatenated_videos_directory: Path, video_extension: str = VideoExtension.MP4
) -> list[Day]:
    videos = load_videos(concatenated_videos_directory, video_extension)
    return [
...
def concatenate_videos(
    day: Day,
    concatenated_videos_directory: Path,
    video_extension: str = VideoExtension.MP4,
) -> Path:
